...
import { WrittenPageNumberingValidator } from "./validators/writtenValidators";
import {
  PitchStructureValidator,
  PitchDimenstionValidator,
} from "./validators/pitchValidators";
...
export async function checkPDF(
  ...
  let textExtractor = new PdfJsTextExtractor();
  let validator = new PitchDimenstionValidator();
  const textResults = await textExtractor.extractText({ pages: pages });
